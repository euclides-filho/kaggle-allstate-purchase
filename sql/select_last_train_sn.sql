SELECT FDATA.* FROM 
	( 
		SELECT customer_ID, max(shopping_pt) shopping_pt FROM all_state. TRAIN_TRUNC TT  where sample_num = %s GROUP BY customer_ID
	)   MAX_V,
	(
		SELECT 
